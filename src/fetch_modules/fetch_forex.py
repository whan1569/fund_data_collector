...
class ForexDataFetcher:
    ...
    def fetch_forex_data(self, start_date=None, end_date=None):
        ...
        try:
            ...
            last_fetch = tracker['forex']['last_fetch_date']
            ...
            # 각 통화쌍별 데이터 수집
            for symbol in tracker['forex']['symbols']:
                try:
            ...
            if all_data:
                ...
                # 기존 데이터가 있다면 병합
                existing_file = self.data_dir / 'forex.parquet'
                if existing_file.exists():
                    existing_df = pd.read_parquet(existing_file)
                    df = pd.concat([existing_df, df]).drop_duplicates()
                
                # Parquet 파일로 저장
                df.to_parquet(self.data_dir / 'forex.parquet')
            ...
            logging.error(f"Error in fetch_forex_data: {str(e)}")
            return False

if __name__ == "__main__":
    fetcher = ForexDataFetcher()
    success = fetcher.fetch_forex_data()
